...
#result, data = mail.uid('search', None, '(UNSEEN FROM "fl0ckfl0ck@hotmail.com")') #특정 주소지로부터 온 메일 중 안읽은 메일만 불러오기
...
for latest_email_uid in data[0].split():
    ...
    try:
        get_email.append(base64.b64decode(get_text(email_message)).decode('ascii')) 
    except:
